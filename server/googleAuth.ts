...
// Memoize Google OIDC configuration
const getOidcConfig = memoize(
  async () => {
    return await client.discovery(
      new URL('https://accounts.google.com/.well-known/openid-configuration'), // Google OIDC discovery endpoint
      clientId,
      clientSecret
    );
  },
...
export async function setupAuth(app: Express) {
  ...
  app.get('/api/login', (req, res, next) => {
    passport.authenticate(`googleauth:${googleDomain}`, {
      prompt: 'select_account consent', // Prompt user to select account and consent
