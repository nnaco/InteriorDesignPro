...
export function ChatWindow({ conversationId, recipientId }: ChatWindowProps) {
  ...
  const recipientInitials = `${recipient.firstName?.[0] || ""}${recipient.lastName?.[0] || ""}`;
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Avatar className="w-10 h-10">
              <AvatarImage src={recipient.profileImageUrl} alt={recipientName} />
      ...
      <CardContent className="flex-1 p-4 overflow-y-auto space-y-4">
        {messages.length === 0 ? (
          ...
          messages.map((msg: any) => {
            ...
            const senderInitials = sender ? `${sender.firstName?.[0] || ""}${sender.lastName?.[0] || ""}` : "?";

            return (
              <div key={msg.id} className={`flex space-x-3 ${isOwn ? "justify-end" : ""}`}>
                {!isOwn && (
                  <Avatar className="w-8 h-8">
                    <AvatarImage src={sender?.profileImageUrl} alt={senderName} />
